...
def find_all(parameter, iterable): #retorna uma lista com os índices de todas as instâncias de parameter dentro de um iterável. O parâmetro pode ser um termo ou uma condição
    indexes = []
    i = 0
...
def filter_by_name(df, name_column_name, parameter): #entrega o dataframe filtrado por nome. O parâmetro pode ser uma string ou uma lista de strings
    name_column_content = df[name_column_name]       #identifique o nome da coluna em name_column_name
    columns = df.columns
    indexes = []
...
def filter_by_row_and_column(df, rows = None, columns = None): #filtra por linhas e colunas. Use tuplas de dois elementos se se referir
    if rows == None:                                           #a todos os elementos do primeiro índice informado ao segundo(inclusivo). Use listas
...
def sort(df, column_name, key = lambda x : x[0], reverse = False):  #sorteia de pelo critério fornecido e retorna um novo dataframe
    duples = []
    indexes = []
    for i in range(len(df)):
        duples.append([df[column_name][i], i])
    duples.sort(reverse = reverse, key = key)
    for x in duples:
        indexes.append(x[1])
    new_df = filter_by_row_and_column(df, indexes)
    return new_df
...
def to_hours(df, time_column_name):  #converte a coluna de horas(formato hh:mm) em unidades de hora(arredondado para 3 casa decimais)
    new_column = []
    for i in range(len(df)):
        hours = int(df[time_column_name][i][-5: -3]) #hora em inteiro(horas e minutos são extraídos por string slicing)
        minutes = int(df[time_column_name][i][-2:]) #análogo
